...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Dashboard /</span> Update Artikel
            {{ $showArtikel->judul }}</h4>
        <div class="card mb-4">
            <h5 class="card-header">Update Artikel</h5>
            <form action="{{ route('dashboardArtikelUpdate', [$showArtikel->id]) }}" method="POST"
                enctype="multipart/form-data" class="card-body">
                @csrf
                <div class="mb-3">
                    <label for="exampleFormControlInput1" class="form-label">Judul Berita</label>
                    <input type="text" name="judul" value="{{ $showArtikel->judul }}"
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="keyword" value="{{ $showArtikel->keyword }}"
                ...
                <div class="mb-3">
                    ...
                    <textarea name="deskripsi" class="form-control  @error('deskripsi') border-danger  @enderror"
                        id="exampleFormControlTextarea1" rows="3">{{ $showArtikel->deskripsi }}</textarea>
                    @error('deskripsi')
                        <div class="text-danger">
                            {{ $message }}
                        </div>
